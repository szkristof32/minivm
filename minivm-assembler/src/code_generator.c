...
static void _code_generator_generate_db(code_generator_t* generator, node_db_t* node);
static void _code_generator_generate_integer(code_generator_t* generator, node_integer_t* node);
static void _code_generator_generate_string(code_generator_t* generator, node_string_t* node);
...
void code_generator_generate(node_generic_t* nodes, size_t node_count, node_t first_node, code_generator_t* out_generator)
{
	...
	while (current_node != -1)
	{
		...
		switch (node->node_type)
		{
			...
			case node_type_db:	_code_generator_generate_db(out_generator,  (node_db_t*)&node->_padding);  break;
			default: continue;
		}
...
void _code_generator_generate_lit(code_generator_t* generator, node_lit_t* node)
{
	...
	_code_generator_generate_integer(generator, integer);
}
...
void _code_generator_generate_db(code_generator_t* generator, node_db_t* node)
{
	node_t current_operand = node->operands;
	for (int i = 0; i < node->operand_count; i++)
	{
		node_generic_t* operand = (node_integer_t*)&generator->nodes[current_operand];
		switch (operand->node_type)
		{
			case node_type_integer:	_code_generator_generate_integer(generator, (node_integer_t*)&operand->_padding); break;
			case node_type_string:  _code_generator_generate_string(generator, (node_string_t*)&operand->_padding); break;
		}
		current_operand = operand->next_node;
	}
}

void _code_generator_generate_integer(code_generator_t* generator, node_integer_t* node)
{
	_code_generator_resize_code(generator, generator->code_size + 1);

	uint8_t radix = node->value->data[0];
	char* number = &node->value->data[1];
	generator->code[generator->code_index++] = strtol(number, NULL, radix);
}

void _code_generator_generate_string(code_generator_t* generator, node_string_t* node)
{
	size_t length = strlen(node->value->data);
	_code_generator_resize_code(generator, generator->code_size + length);
	memcpy(&generator->code[generator->code_index], node->value->data, length);
	generator->code_index += length;
